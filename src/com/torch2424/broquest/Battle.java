...
import com.torch2424.battlequest.CustomListview;
import com.torch2424.battlequest.FontCache;
import com.torch2424.battlequest.GameTextView;
import com.torch2424.battlequest.Level;
...
public class Battle extends Activity 
{
		...
		ServiceConnection musicConnection = new ServiceConnection()
		{
		  ...
		  public void onServiceConnected(ComponentName name, IBinder service) 
		  {
		    ...
		    if(musicBound)
		    {
		    	...
		    	{
		    	bgMusic.playSong(battleMusic[world - 1]);
		    	}
		    }
		  }
	...
	public void getEquip()
	{
		...
		else if(weapon.contains("Durable") || weapon.contains("Good"))
		{
			strength = strength + 3;
		}
	...
	public void getEnemy()
	{
		...
		//get level doing weird thing to round up
		...
		eHP = 5 + (elevel * 2) + generate;
		estrength = 5 + (elevel * 3) + ran.nextInt(3);
		...
		edexterity = generate + (generate/2);
	...
	public void turn()
	{
		...
		else if(HP <= 0 && eHP <= 0)
		{
			...
			{
				//getting exp
				expGain = generate + elevel;
				exp = exp - expGain;
				
				//getting money
				guapGain = generate + ran.nextInt(3);
			...
			else
			{
				...
				//base guap Loss is 3 times world
				guapLoss = (3 * world);
				if(guapLoss > guaps)
				{
					guapLoss = guaps;
	...
	public void fight(View view)
	{
		...
		if(counter)
		{
			//in case the enemy dodges do nothing
			if(edodge)
			{
				damage = 0;
			}
	...
	public void run(View view)
	{
		...
		else
		{
			runFail = false;
			//run succesful
			if(ran.nextInt(100) + 1 >= (60 - dexterity) )
			{
	...
	public void quit(View view)
	{
		...
		//just finish, we already have a world behind us
	...
	//method to save if you lose
			...
			public void save() throws IOException
			{
				...
				if(Integer.valueOf(lastTemp) == generate)
				{
					...
					console.animateText(temptext + "\n\nThe next level has been opened!");	
			...
			public void levelUp() throws FileNotFoundException
			{
				...
				//so now times 20
				fileStream.println( ((level + 1) * 20) - ((generate + elevel) - exp) );
				sc.nextLine();
				//finish skill points
				String temp = sc.nextLine();
	...
	//on pause and on resume to pause and play music
		...
		public void onResume() 
		{
		    ...
		    //test to see if I can get animation working on device
		...
		{
			super.onDestroy();
			//stop any remaining soundpool sounds
